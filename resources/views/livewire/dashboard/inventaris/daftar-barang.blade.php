    ...
    <div class="">
        <div class="">
            <x-dashboard.breadcrumb>
                ...
                <a href="#" class="text-dark-primary hover:text-primary-700">{{ $jenisInventaris->nama }}</a>
                <span class="mx-2">/</span>
                <a href="#" class="text-dark-primary hover:text-primary-700">Daftar Barang</a>
            </x-dashboard.breadcrumb>
        </div>
    </div>
    <div class="w-full 2xl:w-9/12">
        <div class="px-8 py-8 mb-6 bg-white rounded">
            <h4 class="mb-2 font-medium subheading-6 text-primary-700">{{ $jenisInventaris->nama }}</h4>
            <p class="text-base text-dark-secondary">{{ $jenisInventaris->keterangan }}</p>
            <div class="">
                <div class="flex gap-x-4">
                    <p class="">Penanggung Jawab :</p>
                    <p class="font-medium">{{ $jenisInventaris->penanggung_jawab }}</p>
                </div>
            </div>
        </div>
        <div class="mb-6">
            @can('buat-inventaris')
                <div class="">
                    <livewire:dashboard.inventaris.create :jenisInventaris="$jenisInventaris" />
                </div>
            @endcan
        </div>
        <div class="flex flex-col items-baseline justify-between mb-8 lg:flex-row lg:items-end gap-x-8 gap-y-4">
            <div class="flex flex-col md:flex-row gap-y-4 gap-x-8">
                <h3 class="font-semibold subheading-5">Daftar Barang</h3>
            </div>
            ...
            <div class="">
                <div class="">
                    <div class="flex flex-col w-full overflow-x-auto">
                        <table class="w-full overflow-x-auto text-sm font-normal text-left">
                            <thead class="overflow-hidden rounded">
                                <tr
                                    ...
                                    <th scope="col" class="w-64 py-3 pl-1">Nama Barang</th>
                                    <th scope="col" class="w-32 py-3 pl-2">Jumlah</th>
                                    <th scope="col" class="w-40 py-3 pl-2">Kondisi</th>
                                    <th scope="col" class="w-40 px-2 py-3">Pemegang</th>
                                    <th scope="col" class="w-24 px-2 py-3">Keterangan</th>
                                    @if (auth()->user()->can('edit-inventaris') ||
                                            auth()->user()->can('hapus-inventaris'))
                                        <th scope="col" class="w-40 px-2 py-3 text-center">Aksi</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                @if ($daftarBarang->count() > 0)
                                    @foreach ($daftarBarang as $barang)
                                        <tr
                                            ...
                                            {{-- nama barang --}}
                                            <td class="flex items-center justify-start w-64 py-2 pl-1">
                                                <p class="font-medium text-dark-secondary">
                                                    {{ $barang->nama_inventaris }}</p>
                                            </td>
                                            {{-- jumlah --}}
                                            <th scope="col" class="w-32 py-3 pl-2">
                                                {{ $barang->jumlah }}
                                            </th>
                                            {{-- kondisi --}}
                                            ...
                                            {{-- keterangan --}}
                                            <td class="w-24 px-2 py-2 whitespace-nowrap">
                                                <button type="button" class="text-info-700 btn-secondary-small"
                                                    data-te-toggle="modal"
                                                    data-te-target="#keteranganBarang{{ $barang->id }}"
                                            ...
                                            {{-- aksi --}}
                                            <td class="flex w-40 py-2 pl-2 text-sm gap-x-3 whitespace-nowrap">
                                                @can('edit-inventaris')
                                                    <button type="button" wire:click="setEdit({{ $barang }})"
                                                        class="text-warning-700 bg-warning-200 btn-secondary-small"
                                                        data-te-toggle="modal" data-te-target="#editItemModal"
                                                        data-te-ripple-init data-te-ripple-color="light">
                                                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                                            width="24" height="24"
                                                ...
                                                @can('hapus-inventaris')
                                                    <button type="button" wire:click="setDelete({{ $barang }})"
                                                        class="bg-danger-200 btn-secondary-small" data-te-toggle="modal"
                                                        data-te-target="#deleteItemModalId{{ $barang->id }}"
                                                        data-te-ripple-init data-te-ripple-color="light">
                                                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                                            width="24" height="24"
                                            ...
                                            <div data-te-modal-init
                                                ...
                                                id="keteranganBarang{{ $barang->id }}" tabindex="-1"
                                            ...
                                            <div data-te-modal-init wire:ignore
                                                ...
                                                id="deleteItemModalId{{ $barang->id }}" tabindex="-1"
                                                aria-labelledby="exampleModalCenterTitle" aria-modal="true"
                                                role="dialog">
                                                <div data-te-modal-dialog-ref
                                                    class="pointer-events-none relative flex min-h-[calc(100%-1rem)] w-auto translate-y-[-50px] items-center opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:min-h-[calc(100%-3.5rem)] min-[576px]:max-w-[500px]">
                                                    <div
                                                        ...
                                                        <div class="relative p-4">
                                                            <p>Apakah anda yakin ingin menghapus Jenis Catatan "<span
                                                                    class="font-medium">{{ $barang->nama_inventaris }}</span>"?
                                                            </p>
                                                        </div>
                                                        <!--Modal footer-->
                                                        <div
                        ...
                        <div data-te-modal-init wire:ignore
                            class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
                            id="editItemModal" tabindex="-1" aria-labelledby="exampleModalCenteredScrollable"
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        <div class="">
                                            ...
                                            <h6 class="text-dark-secondary">untuk keuangan nama
                                            </h6>
                                        </div>
                                        <!--Close button-->
                                        <button type="button"
                                    ...
                                    <livewire:dashboard.inventaris.edit />
                                </div>
                            </div>
                        </div>
                    </div>
